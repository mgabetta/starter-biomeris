...
run_id <- paste0("TEST_FULL", format(Sys.Date(), "%y%m%d"),"_1")
...
############################################ PS-CTRL ############################################ 
# Save the current completeness status (Complete, Incomplete) 
ps_ctrl <- redcap_data %>% select(c("a01_id", "redcap_event_name", "redcap_repeat_instrument", "redcap_repeat_instance", "a07_complete", "b24_complete","c28_complete",
                             "d34_complete", "e57_complete", "f107_complete", grep("g[[:digit:]]+_119_complete", colnames(redcap_data), value = TRUE), "h06_complete", "i07_complete"))
...
data_rule24 <- ddply(subset(redcap_data, e57_complete == 1 & d34_complete == 1 & !(is.na(e30_ct)) & !(is.na(e31_cn)) & !(is.na(e33_cm)) & !(is.na(e34_cstage)) & e34_cstage != 999), c("a01_id"), summarize,
                       category = "ERROR",
...
data_rule25 <- ddply(subset(redcap_data, e57_complete == 1 & d34_complete == 1 & (e33_cm == 2 | e34_cstage == 8 | ((d11_siterare == 2 | d11_sitecomrar == 2) & e34_cstage == 7) | (d11_sitecomrar == 8 & d27_p16 == 1 & e34_cstage == 5))), c("a01_id"), summarize,
                       category = "ERROR",
...
if(properties$id_centro == id_aiocc) {
  ...
  for (m in dags) {
    ...
    core_pts <- redcap_data_m %>% subset(redcap_event_name == "non_repeatable_arm_1")
    ...
    core <- data.frame("patients" = length(unique(redcap_data_m$a01_id)), "pts_core" = nrow(core_pts),
                       "perc" = formattable::percent(nrow(core_pts)/length(unique(redcap_data_m$a01_id))))
