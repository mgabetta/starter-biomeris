...
run_id <- paste0("TEST_FULL", format(Sys.Date(), "%y%m%d"),"_3")
...
if (nrow(ps_pass)) {
  ...
  writeLines(c("No patient has passed all the data quality checks and can therefore be included in the analysis.", 
               "Please check and correct your data where necessary for future analysis."), fileConn)
  close(fileConn)
